...
import type { DetectedProject, Language, Framework, ProjectStructure } from './types.js';
...
const FRAMEWORK_DETECTORS: FrameworkDetector[] = [
  // Frontend
  { name: 'Next.js', type: 'fullstack', deps: ['next'], files: ['next.config.js', 'next.config.ts', 'next.config.mjs'], priority: 90 },
  { name: 'React', type: 'frontend', deps: ['react', 'react-dom'], priority: 50 },
  ...
  { name: 'SvelteKit', type: 'fullstack', deps: ['@sveltejs/kit'], priority: 80 },
  { name: 'Angular', type: 'frontend', deps: ['@angular/core'], priority: 50 },
  { name: 'Astro', type: 'frontend', deps: ['astro'], priority: 70 },
  { name: 'Remix', type: 'fullstack', deps: ['@remix-run/react'], priority: 70 },
  { name: 'Gatsby', type: 'frontend', deps: ['gatsby'], priority: 50 },
  { name: 'Vite', type: 'frontend', deps: ['vite'], files: ['vite.config.ts', 'vite.config.js'], priority: 40 },
  ...
  { name: 'Prisma', type: 'backend', deps: ['prisma', '@prisma/client'], priority: 40 },
  { name: 'Drizzle', type: 'backend', deps: ['drizzle-orm'], priority: 40 },
  
  // Full-stack
  { name: 'Meteor', type: 'fullstack', deps: ['meteor'], priority: 50 },
  { name: 'Redwood', type: 'fullstack', deps: ['@redwoodjs/core'], priority: 60 },
  
  // Testing
  { name: 'Vitest', type: 'library', deps: ['vitest'], files: ['vitest.config.ts', 'vitest.config.js'], priority: 50 },
  { name: 'Jest', type: 'library', deps: ['jest'], priority: 40 },
  { name: 'Playwright', type: 'library', deps: ['@playwright/test'], priority: 40 },
...
const LANGUAGE_DETECTORS: Array<{
  ...
  { name: 'TypeScript', extensions: ['.ts', '.tsx'], manifestFiles: ['tsconfig.json'] },
  { name: 'JavaScript', extensions: ['.js', '.jsx', '.mjs', '.cjs'], manifestFiles: [] },
  { name: 'Python', extensions: ['.py'], manifestFiles: ['requirements.txt', 'pyproject.toml', 'Pipfile', 'setup.py'] },
  { name: 'Go', extensions: ['.go'], manifestFiles: ['go.mod', 'go.sum'] },
  { name: 'Rust', extensions: ['.rs'], manifestFiles: ['Cargo.toml'] },
  { name: 'Java', extensions: ['.java'], manifestFiles: ['pom.xml', 'build.gradle', 'build.gradle.kts'] },
  ...
  { name: 'CSS', extensions: ['.css', '.scss', '.less', '.tailwind'], manifestFiles: [] },
];
...
export async function scanProject(rootDir: string): Promise<DetectedProject> {
  ...
  const testFramework = frameworks.find(f => ['Vitest', 'Jest', 'Playwright', 'Cypress'].includes(f.name))?.name;
...
function detectFrameworks(manifest: Manifest): Framework[] {
...
function analyzeStructure(rootDir: string): ProjectStructure {
  ...
  for (const srcDir of ['src', 'app', 'lib', 'source', 'server', 'client']) {
    ...
    if (existsSync(srcPath) && statSync(srcPath).isDirectory()) {
      try {
        ...
        sourceDirs.push(...srcEntries.filter(e => {
          return statSync(join(srcPath, e)).isDirectory() && !IGNORED_DIRS.has(e);
        }));
      } catch { /* skip */ }
    }
  }
  ...
  return {
    hasSrcDir: existsSync(join(rootDir, 'src')) || existsSync(join(rootDir, 'app')),
    hasTestsDir: topLevelDirs.some(d => d.startsWith('test') || d.startsWith('__test')),
    hasDocsDir: topLevelDirs.includes('docs'),
    hasCiConfig: existsSync(join(rootDir, '.github')) || existsSync(join(rootDir, '.gitlab-ci.yml')),
    hasDockerConfig: existsSync(join(rootDir, 'Dockerfile')) || existsSync(join(rootDir, 'docker-compose.yml')),
    topLevelDirs,
    sourceDirs,
  };
}
...
function detectBuildTool(rootDir: string, manifest: Manifest): string | undefined {
  ...
  if (scriptValues.includes('tsc') || scriptValues.includes('ts-node')) return 'TypeScript';
