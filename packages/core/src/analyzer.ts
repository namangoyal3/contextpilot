...
const IMPORTANT_SECTIONS = [
  { id: 'projectDescription', patterns: ['project', 'overview', '# ', '## '], weight: 15 },
  { id: 'techStack', patterns: ['tech stack', 'technology', 'stack', 'framework', 'language'], weight: 15 },
  { id: 'commands', patterns: ['command', 'npm run', 'yarn', 'pnpm', 'script', 'build', 'test'], weight: 15 },
  { id: 'architecture', patterns: ['architecture', 'structure', 'directory', 'folder', 'src/'], weight: 15 },
  { id: 'conventions', patterns: ['convention', 'style', 'guideline', 'pattern', 'naming'], weight: 15 },
  { id: 'guardrails', patterns: ['guardrail', "don't", 'do not', 'avoid', 'never', 'forbidden'], weight: 15 },
  { id: 'references', patterns: ['reference', 'docs/', 'see also', 'further'], weight: 10 },
];
...
export function analyzeContextFile(filePath: string): ContextAnalysis {
  ...
  const suggestions = generateAnalysisSuggestions(coverage, lines.length, content);
...
function generateAnalysisSuggestions(
  ...
  if (!coverage.projectDescription) {
    suggestions.push({
      ...
      details: 'Add a brief description of what the project does and who it\'s for.',
    });
  }
  ...
  if (!coverage.conventions) {
    suggestions.push({
      ...
      details: 'Specify naming conventions, component patterns, and style preferences.',
    });
  }
  ...
  if (lineCount > 200) {
    suggestions.push({
      ...
      details: 'Consider splitting into multiple files or removing redundant information.',
    });
  }
  ...
  if (lineCount < 15) {
    suggestions.push({
      ...
      details: 'Aim for at least 30-50 lines covering project overview, tech stack, and conventions.',
    });
  }
 ...
 */
export function detectContextFiles(rootDir: string): Array<{ path: string; type: string }> {
  const files: Array<{ path: string; type: string }> = [];
  const candidates = [
    { name: 'CLAUDE.md', type: 'claude-code' },
    { name: '.claude/claude.md', type: 'claude-code' },
    { name: '.cursorrules', type: 'cursor' },
    { name: '.cursor/rules/', type: 'cursor' },
